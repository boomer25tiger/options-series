...
Section 7's implementation figure gates the strip on each signal, and neither gate carries a test. The O1 gate trades a cycle when log(K/RV21) sits at or above the median of the fund's earlier values, while the O2 gate skips a cycle when IV30/IV91 exceeds the 90th percentile of its own history. Each fund's first 36 cycles always trade, and a skipped cycle enters the pooled series as a zero return. Over the estimation window the O1 gate trades 61 percent of strip fund-cycles for a pooled mean of 0.056 per cycle against 0.122 unconditionally, and over the holdout it trades 46 percent for 0.021 against 0.065. The O2 gate trades 92 and 88 percent of fund-cycles in the two windows for 0.124 and 0.051. Per traded fund-cycle, the basis that matches the unconditional fund-cycle means of 0.131 and 0.060, the O1 gate earns 0.100 and 0.048 while the cycles it skips earn 0.181 and 0.071. On the same basis the O2 gate earns 0.133 and 0.055. A slope detects a monotone relationship while a threshold effect would show in the gate, and here both point one way, since the O1 gate selects lower-earning cycles in both windows and the O1 slope of −0.053 also runs negative. That direction runs against Goyal and Saretto (2009), whose result implies a seller should sell when implied variance sits high relative to trailing realized variance.

![Pooled cumulative strip return at k = 0.5 and c = 2 bps, unconditional and under the O1 and O2 gates](items/item3_short_variance/figures/fig4_gates.png)

Sorted into O2 quintiles with no p-value attached, the strip's mean cycle return runs between 0.026 and 0.184 with no ordering, at a rank correlation with O2 of −0.002. For the straddle the quintile means run from −0.012 to 0.045 at a rank correlation of 0.056, and the medians rise from 0.009 in the lowest quintile to 0.053 in the highest, toward the inverted term structure the O2 gate skips.

![Mean and median cycle return by O2 quintile at k = 0.5 and c = 2 bps for both arms](items/item3_short_variance/figures/fig5_o2_series.png)
