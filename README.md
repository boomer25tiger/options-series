...
Section 7's implementation figure gates the strip on each signal, and no test attaches to it. The O1 gate trades a cycle when log(K/RV21) sits at or above the median of the fund's earlier values, while the O2 gate skips a cycle when IV30/IV91 exceeds the 90th percentile of its own history. Each fund's first 36 cycles always trade, and a skipped cycle enters as a zero return. Over the estimation window the O1 gate trades 61 percent of strip fund-cycles for a pooled mean of 0.056 per cycle, against 0.122 unconditionally, and over the holdout it trades 46 percent for 0.021 against 0.065. The O2 gate trades 92 percent of estimation fund-cycles for 0.124. A slope detects a monotone relationship while a threshold effect would show in the gate, and the O1 gate shows none, since the cycles it skips earned their share of the premium. Sorted into O2 quintiles with no p-value attached, the strip's mean cycle return runs between 0.026 and 0.184 with no ordering, and its rank correlation with O2 is −0.002. For the straddle the quintile means run from −0.012 to 0.045 and the rank correlation is 0.056.
